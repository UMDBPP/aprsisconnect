...
class FrameTestCase(aprs_test_classes.APRSTestClass):  # pylint: disable=R0904
    ...
    def test_aprs_frame(self):
        ...
        formatted_frame = aprsisconnect.parse_frame(frame)
    ...
    def test_decode_aprs_frame(self):
        ...
        aprs_frame = aprsisconnect.parse_frame(frame)
    ...
    def test_ax25_encode(self):
        ...
        self.assertEqual(encoded_frame[-1:], aprsisconnect.AX25_FLAG)
        self.assertEqual(encoded_frame[-3:-1], b'\xff\x07')
        self.assertEqual(str(aprsisconnect.parse_callsign_ax25(encoded_frame[1:8])), 'APRX24')
        self.assertEqual(str(aprsisconnect.parse_callsign_ax25(encoded_frame[8:15])), 'W2GMD-6')
        self.assertEqual(str(aprsisconnect.parse_callsign_ax25(encoded_frame[15:22])), 'WIDE1-1')
        self.assertEqual(str(aprsisconnect.parse_callsign_ax25(encoded_frame[22:29])), 'WIDE2-1')
        self.assertEqual(encoded_frame[29:31], aprsisconnect.ADDR_INFO_DELIM)
        self.assertEqual(
            encoded_frame[31:-3],
            bytearray(b'!3745.75NI12228.05W#W2GMD-6 Inner Sunset, SF iGate/Digipeater http://w2gmd.org')
        )

        decoded_frame = aprsisconnect.parse_frame(encoded_frame)
    ...
    def test_ax25_decode(self):
        ...
        decoded_frame = aprsisconnect.Frame(encoded_frame)
